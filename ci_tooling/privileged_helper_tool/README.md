...
`com.ybronst.opencore-legacy-patcher.privileged-helper` is OpenCore Legacy Patcher's Privileged Helper Tool.
...
2. The privileged helper tool will check the code signature of the main application to ensure it is signed by ybronst.
...
* Note that ybronst's Team ID will need to be replaced in main.m with your own Team ID (`Q552RWTDRT` -> `YOUR_TEAM`)
